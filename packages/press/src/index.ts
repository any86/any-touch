...
import {
    STATUS_FAILED, STATUS_RECOGNIZED, STATUS_END, STATUS_CANCELLED, STATUS_POSSIBLE, DIRECTION_UP, INPUT_CANCEL, INPUT_END, INPUT_START
...
export default function (context: Core, options?: Partial<typeof DEFAULT_OPTIONS>) {
    ...
    context.on('computed', (computed) => {
        ...
        if (INPUT_START === phase && _options.pointLength === pointLength) {
            // 重置状态
            // 重置status
        ...
        // 1. end阶段
        // 2. 已识别
        else if (INPUT_END === phase && STATUS_RECOGNIZED === status) {
            ...
            if (!test(computed, _options) ||
                ...
                (_options.minPressTime > deltaTime && [INPUT_END, INPUT_CANCEL].includes(phase))) {
                clearTimeout(timeoutId)
